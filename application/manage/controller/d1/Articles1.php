...
//                    "a_type.in" => "选择的类型有误!~",
    ...
    {
        if ($this->request->isPost()) {
            $add_info = $this->request->only(['bi_type', 'r_id', 'bi_title', 'bi_winning_bid', 'bi_winning_bid_time', 'bi_start_time', 'bi_is_show', 'bi_contents', 'bi_is_pay', "bi_desc",'bi_end_time','bi_agent','bi_proprietor_com','bi_industry'], 'post');
            /*验证接到的值有没有问题*/
            $rule = array(
    ...
    public function biddingInformationEdit($id)
    {
        if (isset($id) && $id > 0) {
            ...
            if (!empty($bi_info)) {
                if ($this->request->isPost()) {
                    $save_info = $this->request->only(['bi_type', 'r_id', 'bi_title', 'bi_winning_bid', 'bi_winning_bid_time', 'bi_start_time', 'bi_is_show', 'bi_contents', 'bi_is_pay', "bi_desc",'bi_end_time','bi_agent','bi_proprietor_com','bi_industry'], 'post');
                    /*验证接到的值有没有问题*/
                    $rule = array(
                        "bi_type" => 'require|in:0,1,2',
                        "r_id" => 'number',
